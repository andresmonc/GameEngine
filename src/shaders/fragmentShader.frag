...
void main(void) {
	...
	for (int i = 0; i < 4; ++i) {
        ...
        totalDiffuse = totalDiffuse + (brightness * lightColor[i])/ attFactor;// divide (brightness/lightcolor) by attenuation factor to decrease brightness as distance from light increases
        ...
        totalSpecular = totalSpecular +  (dampedFactor * reflectivity * lightColor[i]) / attFactor;// divide (brightness/lightcolor) by attenuation factor to decrease brightness as distance from light increases
	}
