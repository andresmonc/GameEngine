...
void main(void){
	...
	for (int i = 0; i < 4;){
		...
		float brightness = max(nDot1, 0.0);//can you remove this?
	...
	out_Color = mix(vec4(skyColour, 1.0), out_Color, visibility);  // mix the skyColor with the current out_Color, by the visibility gradient.
}